    ...
    then
       ...
       then
         bsub -q week -J $i -o. /log_files/error_sim_${names[$i]}_$j.log\
           python spat_community_generation.py ${S[$i]} $N[$i]} 200 ${bisect[$i]} False None ${names[$i]} 
       fi
       if [ $j == empirSAD ]
       then
         bsub -q week -J $i -o. /log_files/error_sim_${names[$i]}_$j.log\
           python spat_community_generation.py ${S[$i]} $N[$i]} 200 ${bisect[$i]} False\
           ../data/${names[$i]}_sad.csv $(names[$i]}
       fi
    fi
    if [ $1 == usu ]
    then
