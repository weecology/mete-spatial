...
if len(sys.argv) > 1:
    ...
    bisect_fine = int(sys.argv[2]) 
    bisect_coarse = int(sys.argv[3])
    sadType = sys.argv[4]
    ...
    print 'Error: Specify psi, bisect_fine, bisect_coarse, sadType, abu file path, and out filepath at the command line'
...
if sadType == 'meteSAD':
    S0 = len(abu)
    N0 = sum(abu)
    sor = [mete.sor_bisect(A, A0, S0, N0, psi, shape, unit_distance) for A in A_vals]
    sor = np.concatenate((sor), axis=0)
else:
    sor = [mete.sor_bisect_fixed_abu(A, abu, A0, psi, shape, unit_distance) for A in A_vals]
    sor = np.concatenate((sor), axis=0)

writer = open(out_filepath, 'wb') 
datawriter = csv.writer(writer)
 
datawriter.writerow(['psi', 'i', 'j', 'dist', 'sor'])
