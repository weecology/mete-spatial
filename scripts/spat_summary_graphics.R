...
##scale 2D distances
distsGrid = distsGrid #* (max(distsGrid)/max(distsGrid))
vGridExpAvg = apply(vGridExp,1,mean)
vGridExpQt = apply(vGridExp,1,function(x)quantile(x,c(.025,.975)))

par(mfrow=c(1,2))
plot(distsGrid,vGridExpAvg,type='n',ylim=range(vGridExpAvg,vGridExpQt),
...
##
plot(log10(distsGrid),log10(vGridExpAvg),type='n',ylim=log10(range(vGridExpAvg,
     vGridExpQt)),ylab='log10 Variance Within-species',xlab='log10 Spatial Lag',
...
##sorensen
load('./sorensen/sorensen_S100_N10000_C200_B11_grid_binary_old.Rdata')
...
polygon(c(distsGrid,distsGrid[length(distsGrid):1]),
        c(vGridExpQt[1,],vGridExpQt[2,length(distsGrid):1]),col='lightblue',
        border = NA)
lines(distsTran,vTranExpAvg,type='l',col='red',pch=19,lwd=8)
lines(distsGrid,vGridExpAvg,type='l',col='blue',pch=19,lwd=2)
...
polygon(log10(c(distsGrid,distsGrid[length(distsGrid):1])),
        log10(c(vGridExpQt[1,],vGridExpQt[2,length(distsGrid):1])),
        col='lightblue',border = NA)
lines(log10(distsTran),log10(vTranExpAvg),type='l',col='red',pch=19,lwd=8)
lines(log10(distsGrid),log10(vGridExpAvg),type='l',col='blue',pch=19,lwd=2)
